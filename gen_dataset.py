...
for id in range(1, 801):
    ...
    backSub = cv2.createBackgroundSubtractorKNN(
        history=200, dist2Threshold=800, detectShadows=False
    )
    ...
    while cap.isOpened():
        ...
        if (
            current_frame >= data.loc[0, 'HitFrame'] + 5
            and current_frame <= data.loc[len(data) - 1, 'HitFrame']
        ):
            if len(ball_contours) != 0:
                ...
                y_ = y_ + int(h_ / 2)
                distance = int(
                    math.sqrt(pow(x_ - last_coordinate[0], 2) + pow(y_ - last_coordinate[1], 2))
                )
                for cnt in ball_contours:
                    ...
                    _y = y + int(h / 2)
                    curr_distance = int(
                        math.sqrt(pow(_x - last_coordinate[0], 2) + pow(_y - last_coordinate[1], 2))
                    )
                    print(curr_distance)
                    if curr_distance <= distance:
                        ...
                        if curr_distance <= max_distacne or nn >= MaxTryToTrack:
                            ...
                            cv2.circle(
                                framecp, (int(x + w / 2), int(y + h / 2)), 5, (0, 0, 255), -1
                            )
                            cv2.imshow('roi', cv2.resize(roi, (w * 5, h * 5)))
                            if prue_csv_mode == False:
                                cv2.imwrite(
                                    F'{dataset_dir}/{id}/{current_frame}_{_x}_{_y}.jpg', roi
                                )
