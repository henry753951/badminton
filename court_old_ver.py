...
for id in range(16, 801):
    ...
    if ret:
        ...
        corners = cv2.goodFeaturesToTrack(gray, 200, 0.03, 15)
        ...
        for i in range(1, len(corners)):
            x,y = corners[i].ravel()
            cv2.circle(frame,(x,y),3,(40,50,100),-1)
            onBorder = False
        ...
        corners_useGray = np.array(filter_corners)

        #  霍夫直線變換
        blur = cv2.GaussianBlur(edges, (3,3), 0)
        edges = cv2.addWeighted(edges, 0.5, blur, 0.5, 0)
        lines = cv2.HoughLinesP(
                edges, rho=1 , theta=np.pi / 110, threshold=50, minLineLength=50, maxLineGap=10
        )
        for line in lines:
            x1, y1, x2, y2 = line[0]
            cv2.line(court, (x1, y1), (x2, y2), (80, 20, 210), 4)

        gray2 = cv2.cvtColor(court, cv2.COLOR_BGR2GRAY)
        corners_useHough = cv2.cornerHarris(gray2, 5, 9, 0.04)
        corners_useHough = cv2.dilate(corners_useHough, None)
        ret, corners_useHough = cv2.threshold(corners_useHough, 0.01 * corners_useHough.max(), 255, 0)
        corners_useHough = np.uint8(corners_useHough)
        ret, labels, stats, centroids = cv2.connectedComponentsWithStats(corners_useHough)
        criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 100, 0.1)
        corners_useHough = cv2.cornerSubPix(gray2, np.float32(centroids), (5, 5), (-1, -1), criteria)

        points = []
        for i in range(1, len(corners_useHough)):
            x,y = corners_useHough[i]
            x = int(x)
            y = int(y)
            onBorder = False
        ...
        corners_useHough = np.array(points)


        h_lines = []
        max_length_of_line = 0
        max_y = 0
        for i in range(len(corners_useGray)):
            for j in range(i+1, len(corners_useGray)):
                x1, y1 = corners_useGray[i]
                x2, y2 = corners_useGray[j]
        ...
        for line in h_lines:
            x1, y1, x2, y2 = line
            if y1 <= max_y+5 or y2 <= max_y+5:
                temp.append(line)
        h_lines = temp
        temp=[]
        
        for point in corners_useGray:
            if point[1] <= max_y+5:
                temp.append(point)
        corners_useGray = temp
        ...
        for line in h_lines:
            x1, y1, x2, y2 = line
            if pow(pow(x1 - x2, 2) + pow(y1 - y2, 2), 0.5) >= 100:
                cv2.line(frame, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 255), 2)
                cv2.circle(frame,(x1,y1),3,(100,200,100),3)
                cv2.circle(frame,(x2,y2),3,(100,200,100),3)
                temp.append(line)
                h_lines_points.append([x1,y1])
                h_lines_points.append([x2,y2])
        h_lines = temp

            

        for p in h_lines_points:
            sorted_points = sorted(corners_useHough, key=lambda x: pow(pow(x[0] - p[0], 2) + pow(x[1] - p[1], 2), 0.5))
            try:
